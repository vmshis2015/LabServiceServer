...
{
    internal class Hitachi717_XD : Rs232Base
    {
            ...
            try
            {
                ...
                foreach (string patient in allPatients)
                {
                    ...
                    if (Validdata(patient))
                    {
                        ...
                        {
                            //BIL-Toàn phần
                            if (item.TestName.Equals("6")) iBilt = TryToConvertToDouble(item.TestValue);

                            //BIL-Trực tiếp
                            if (item.TestName.Equals("7")) iBild = TryToConvertToDouble(item.TestValue);

                            //Protein-T                                    
                            if (item.TestName.Equals("3")) iProT = TryToConvertToDouble(item.TestValue);

                            //ALB                                   
                            if (item.TestName.Equals("4")) iAlb = TryToConvertToDouble(item.TestValue);

                            //CHOL
                            if (item.TestName.Equals("10")) iChol = TryToConvertToDouble(item.TestValue);

                            //Trig
                            if (item.TestName.Equals("11")) iTrig = TryToConvertToDouble(item.TestValue);

                            //HDLC
                            if (item.TestName.Equals("12")) iHdlc = TryToConvertToDouble(item.TestValue);
                        }
