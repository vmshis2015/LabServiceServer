...
{
    internal class GA400Xd : Rs232AstmDevice
    {
        /// <summary>
            ...
            try
            {
                ...
                foreach (string patient in arrPatients)
                {
                    ...
                    foreach (ResultItem item in TestResult.Items)
                    {
                        ...
                        //Protein-T                                    
                        if (item.TestName.ToUpper().Equals("PROTEIN T.P")) iProT = TryToConvertToDouble(item.TestValue);

                        //ALB                                   
                        if (item.TestName.ToUpper().Equals("ALB")) iAlb = TryToConvertToDouble(item.TestValue);

                        //CHOL
                        if (item.TestName.ToUpper().Equals("CHOLESTEROL")) iChol = TryToConvertToDouble(item.TestValue);

                        //Trig
                        if (item.TestName.ToUpper().Equals("TRIGLYCERID")) iTrig = TryToConvertToDouble(item.TestValue);
